...
Obsidian 插件：把固定格式的文本一键转换成 `/` `\` 斜杠风格的二叉树字符画，专为算法笔记设计。输入可以是「每行一层」的层级写法、「父节点: 左 右」的父子写法、LeetCode 层序数组或缩进列表，输出是算法题里常见的左右斜杠画法，而不是 `├──` 那种目录树。
...
格式自动识别，哪种顺手用哪种；弹窗里会显示识别到的格式。

### 1. 层级格式：每行一层（推荐）

第一行写根，之后每行写一层。用 `|` 把孩子按父节点分组，顺序和上一行一致，`_` 表示空位。
...
- 第 3 行有两组：`9` 的孩子（没有）和 `20` 的孩子（`15 7`）。
- 每组最多两个值：`左 右`。`_ 7` 表示只有右孩子，单写 `15` 表示只有左孩子，空组表示没有孩子。
- 上一层每个节点正好对应一组，组数不对会立刻报错并列出上一层的节点，不会悄悄错位。
- 不写 `|` 时，每个父节点必须正好写两个值，和 LeetCode 数组一样：`4 5 _ 6`。

再看一个大一点的例子，对应 `[5,4,8,11,null,13,4,7,2,null,null,null,1]`：
...
每行写一个有孩子的节点：`父节点: 左 右`。第一行的父节点是根，后面每行的父节点必须已经在前面某行里作为孩子出现过。叶子不用写。
...
有同名节点时，每行指的是按出现顺序第一个还没指定过孩子的那个，所以一层一层往下写就是对的。
